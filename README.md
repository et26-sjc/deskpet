...
`Love Roommate` 是一个面向 **Codex Desktop** 的方法型 Skill：它负责诊断照片、让用户认领自己、提供行为模式、指导生成素材、检查效果并构建应用。仓库本身不包含用户提交的照片或生成角色图；仅包含作者本人主动公开的社区联系方式二维码。
...
想交流 Codex、桌宠制作和 Skill 开发，可以扫描下面的微信二维码添加作者。添加时请备注 **“Codex 学习群”**，通过好友后我会邀请你进入学习群。

> 这是作者个人维护的非官方学习交流群，不属于 OpenAI 或 Codex 官方社群。请勿在群内发布他人隐私、未授权照片或敏感信息。

以下两个二维码及其中的作者头像由作者本人主动公开，仅用于添加作者微信、邀请进入学习群和关注作者的小红书账号。二维码文件使用固定路径和 SHA-256 接受发布审计，替换或增加其他个人图片会使隐私门禁失败。

### 通过微信添加好友并进入学习群

<img src="assets/community/wechat-learning-group.jpg" alt="作者微信二维码，添加好友后邀请进入 Codex 学习群" width="420">

### 小红书：维天说

<img src="assets/community/xiaohongshu-wetianshuo.jpg" alt="作者小红书账号维天说二维码" width="420">
...
- `屎追逐（有本人）`：整队只缓慢移动到安全位置一次，之后固定；普通鼠标移动不影响任何人。本人是唯一且持续拉屎的人，其余全员轮流追吃，吃完归队，身份永不交换。只有主动拖拽本人时才整体重定位，松手后固定。
- `屎追逐（本人不在）`：鼠标控制一坨点击穿透的屎，照片里全员首尾连接成人形蜈蚣，使用限速、加速度和死区平滑追着爬，禁止瞬移。
...
3. 有本人时，本人是站立的“被膜拜者”，不会跪、不会喊、不会显示对应气泡；其他角色全部跪喊。屎追逐里整队只执行一次缓慢安全落位，随后普通鼠标移动不影响任何人；本人持续拉，其他角色全部追吃，吃的人不会反过来成为拉的人。只有拖拽本人时整队与当前屎整体重定位，松手后固定。
4. 本人不在照片时，不虚构一个“主人”：爸爸/爷爷模式全员跪喊；屎追逐由鼠标控制点击穿透的屎，全员组成人形蜈蚣，使用限速、加速度和死区平滑追随，禁止瞬移。
5. 跪喊参与者在空间足够时居中单排；空间不足时自动组成不重叠、不裁切的居中多排。有本人时本人站在队列正前方中心轴；无本人时整队居中。
6. 第一次确认检查脸、发型、衣服、编号、本人和模式；第二次确认检查动作、完整身体和透明边缘。
7. 运行阶段会分支检查：有本人时普通鼠标移动不改变固定队形、只有拖拽本人才能整体重定位；本人不在时才检查鼠标屎与平滑蜈蚣追随。两个分支都检查队形连接、“全程只有一坨”、本人是否始终是拉屎来源，以及爸爸/爷爷是否按本人存在状态正确分配全员。屎的透明验收同时使用浅色和深色背景；同一帧还会额外截取隐藏屎窗口后的底图，逐像素检查透明内部、外沿和四角，不能只相信 PNG 自身的 alpha，并自动拒绝白色方块、白边、灰边或光晕。
